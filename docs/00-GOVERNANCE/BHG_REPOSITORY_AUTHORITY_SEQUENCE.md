...
Cross-repository relationships shall distinguish:
...
Future work shall follow this reading and dependency sequence:
...
Not every document depends directly on every layer. The sequence is a resolution order: when a new document is created, its applicable superior authority must be identified before its local scope is defined.
...
Owns institutional and ecosystem architecture inside its approved scope. Its architecture must remain compatible with the BHG Constitution and the canonical governance hierarchy.

### BHG-Governance

Owns the governance system, including governance models, policies, documentary standards and compliance mechanisms inside its approved scope.

### bhg-knowledge

Consumes applicable Foundation and Governance contracts. It may define knowledge-specific taxonomy, workflows and services, but these remain subordinate to applicable enterprise contracts.

### ZivaLatam

Consumes applicable Foundation and Governance contracts. Its Engineering Charter, ZES rules, ADRs and implementation contracts are local specializations and must not redefine enterprise governance semantics.
...
This document is an architectural normalization artifact. It becomes an effective cross-repository contract only after the applicable governance approval process is completed.
